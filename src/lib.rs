...
mod tests {
    ...
    fn test_post_and_get_messages() {
        let api = API::from_config(
            &ZulipConfig::from_file(ZULIP_CONFIG_PATH.to_path_buf())
                .expect("Failed to read config"),
        );

        let posted = api
            .post_message(STREAM, TOPIC, "test_post_and_get_messages")
            .expect("Failed to post messages");
        ...
        assert!(
            ...
            "Must receive at least the posted message"
        );
    }
    ...
    fn subscribe_to_messages() {
        ...
        api.post_message(STREAM, TOPIC, message)
            .expect("Failed to post message");
        let messages = api
            .get_queued_messages(&mut queue)
            .expect("Failed to retrieve queued messages");
        assert!(
            messages.len() >= 1,
            "Must receive at least the posted message"
        );
    ...
    fn work_with_streams() {
        ...
        api.create_stream("test zulip-rs stream", false)
